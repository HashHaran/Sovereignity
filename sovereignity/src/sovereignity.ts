...
export function handleContentCreation(event: ContentCreation): void {
  ...
  let contentEntity = new Content(event.params.contentId.toString()); //Content ID is an unique identifier in this table
...
export function handleContentDeletion(event: ContentDeletion): void {
  store.remove('Content', event.params.contentId.toString());
}

export function handleOwnershipTransfered(event: OwnershipTransfered): void {
  let contentEntity = Content.load(event.params.contentId.toString());
  if (contentEntity != null) {
    contentEntity.owner = event.params.owner;
    contentEntity.save();
  } else {
    console.error("OwnershipTransfered event emitted for content not present in data store.")
  }
}

export function handlePermissionGranted(event: PermissionGranted): void {
  const id = event.params.contentId.concat(event.params.user as Bytes); //Contract makes sure that one user has permission only once to a content id, os it will be unique
  let contentPermissionEntity = new ContentPermission(id);
  contentPermissionEntity.content = event.params.contentId.toString();
...
export function createContentCreationEvent(contentId: string, owner: string, timeStamp: BigInt): ContentCreation {
  ...
  let contentIdParam = new ethereum.EventParam('contentId', ethereum.Value.fromString(contentId));
